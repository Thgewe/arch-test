...
export const getTimeSpan = (date: string | undefined) => {
    ...
    if (year !== currentTime.getUTCFullYear()) {
        timeSpan.years = Math.floor((Math.abs(currentTime.getUTCFullYear() - year) * 12 + currentTime.getUTCMonth() - month) / 12);
    }
    if (month !== currentTime.getUTCMonth()) {
        timeSpan.months = Math.abs(Math.abs(currentTime.getUTCFullYear() - year) * 12 + currentTime.getUTCMonth() - month) - timeSpan.years * 12;
    }
...
export const timeElapsedTest = (date: string, latestDate: Date): string => {
    let str: string;

    const time: ITime = getTimeSpanTest(date, latestDate);
...
export const getTimeSpanTest = (date: string | undefined, latestDate: Date) => {
